...
findJavaHome()
{
  if [[ -f $HOME/.sqldeveloper/23.1.1/product.conf ]];
  then
    IncludeConfFile $HOME/.sqldeveloper/23.1.1/product.conf
  fi
...
if [[ -f $HOME/.sqldeveloper/23.1.1/env.sh ]];
then
  	source "$HOME/.sqldeveloper/23.1.1/env.sh" >> /dev/null
