...
findJavaHome()
{
  if [[ -f $HOME/..oraclesqldeveloperdatamodeler/23.1.0/product.conf ]];
  then
    IncludeConfFile $HOME/..oraclesqldeveloperdatamodeler/23.1.0/product.conf
  fi

  if [ -z "$APP_JAVA_HOME"]; then
    APP_JAVA_HOME=`Resolve ../Resources/datamodeler/jdk`
  fi
}
...
if [[ -f $HOME/.oraclesqldeveloperdatamodeler/23.1.0/env.sh ]];
then
  	source "$HOME/.oraclesqldeveloperdatamodeler/23.1.0/env.sh" >> /dev/null
elif [[ -f $HOME/.oraclesqldeveloperdatamodeler/env.sh ]]; 
then
	source "$HOME/.oraclesqldeveloperdatamodeler/env.sh"  >> /dev/null
fi

here="${0%/*}"
cd "${here}"
cd ../Resources/datamodeler/datamodeler/bin
bash ./datamodeler >>/dev/null
